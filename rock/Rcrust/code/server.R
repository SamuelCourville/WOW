...
shinyServer(function(input, output,session) {
    ...
    if(reactive_message$data=="error handling passed"){
	  ...
	  if(input$component_packet){
		if(input$cp_components=="load"){
			...
			if(!isTRUE(input$cp_components=="")){
				...
				#Fix-tag: tried to make it only show up in text file when component created, but problem with components remaining loaded in workspace
				  new_components<-"\nnew_components<-c(\""
				  count_new_cp<-0
	  ...
	  if(input$n_phases_to_rename=="load"){
        ...
        if(!input$n_phases_to_rename==""){
          #Error validation
          if(is.na(suppressWarnings(as.numeric(input$n_phases_to_rename)))){return("Error: Number of phases to rename must be numeric")}
          if(as.numeric(input$n_phases_to_rename)<1){return("Error: Number of phases to rename must be greater than 0")}
          if(!as.numeric(input$n_phases_to_rename)%%1==0){return("Error: Number of phases to rename must be a whole number")}
          list_phases_to_rename<-NULL
  ...
  observeEvent(input$run, {
    ...
    if(reactive_message$data=="error handling passed"){
      ...
      if(!is.null(var_missing)){reactive_message$data <- paste0("Error: Cannot run calculation. Missing ",var_missing);return(reactive_message$data)}
      #Save
  ...
  reactive_message$data <- paste0(on_load())
    ...
    observe({
      ...
      current_components_r$data<-current_components
		    ...
		    #Component packet new component
    observe({
      #use current component transformation inputs to update current components_r and selection in major elements
      current_components<-available_components_r$current
      chk_trans<-FALSE
...
#remove phases<-unlist(strsplit(chk_Phases,split=\",\"))
	...
	chk_Phases<-try(eval(parse(text=paste0(\'input$cp_phases_\',a))),silent=TRUE)
					...
					phases<-unlist(strsplit(chk_Phases,split=\",\"))
						...
						column(4,textInput(paste0(\'cp_phases_\',a,\'_\',phases[j]),phases[j],value=chk_in_cp_phases))
						}))
						}}
						})")))
	})
    ...
    output$rename_phases_inputs <- renderUI({
      ...
      if(!(is.null(def_num)|def_num==""|def_num==0)){
        validate(if(is.na(suppressWarnings(as.numeric(def_num)))){"Error: Number of phases to rename must be numeric"}else{NULL})
        validate(if(as.numeric(def_num)<1){"Error: Number of phases to rename must be greater than 0"}else{NULL})
        validate(if(!as.numeric(def_num)%%1==0){"Error: Number of phases to rename must be a whole number"}else{NULL})
        fixedRow(
          lapply(1:(as.numeric(def_num)*3), function(i) {
            a<-1; ii<-i
            while(ii>3){ii<-ii-3;a<-a+1}
            if(ii==1){chk_old<-try(unlist(strsplit(names(phases_to_rename_r$data)[a],split="~"))[1],silent=TRUE)
            if(class(chk_old)=="try-error"){chk_old<-NULL}
            column(2,textInput(paste0('old_name_',a),'Old name',value=chk_old))}else{
            if(ii==2){chk_new<-try(unlist(strsplit(names(phases_to_rename_r$data)[a],split="~"))[2],silent=TRUE)
                      if(class(chk_new)=="try-error"){chk_new<-NULL}
                      column(2,textInput(paste0('new_name_',a),'New name',value=chk_new))}else{
            if(ii==3){chk_condition<-try(phases_to_rename_r$data[a],silent=TRUE)
            if(class(chk_condition)=="try-error"){chk_condition<-NULL}
            column(2,textInput(paste0('condition_',a),'Condition',value=chk_condition))}else{
                  }}}})
        )
      }
    })
...
draw_abundance_r<-reactive({
		...
		#Plot
		barplot(phase_abundance_data,space=0,col=cols,border=NA,legend.text=(input$legend!="None"), args.legend=list(x =input$legend,bty = "n"),xlab=input$path_label,ylab=paste("Phase abundance",input$proportion))
		if(action=="Save"){
		title(paste("Grid of ",gsub("\\/"," per ",grid_data_r()[[1]])," for ",input$working_file))
		dev.off()
		}
...
return(paste0("Phase Assemblage Map saved to ",input$projects_directory,"/",input$working_file,"/Outputs/\n"))
    ...
    output$output_selection <- renderUI({
      ...
      conditionalPanel("input.output_type == 'Grid'",
		selectInput('Grid_variable', 'Variable', c("Pressure","Temperature",all_columns,"Custom"), selected="Pressure",selectize=TRUE),
